...
public:
    ...
    BinaryHeap(const BinaryHeap& other) : heap(new Node[other.physicalSize]), virtualSize(other.virtualSize), physicalSize(other.physicalSize) {
        copy(other.heap, other.heap + other.virtualSize, heap);
    }

    BinaryHeap& operator=(const BinaryHeap& other) {
        if (&other != this) {
            Node* newHeap = new Node[other.physicalSize];
            copy(other.heap, other.heap + other.virtualSize, newHeap);
            delete[] heap;
            heap = newHeap;
    ...
    void addValue(size_t priority, const char* data) {
        if (virtualSize == physicalSize) {
            physicalSize *= 2;
            Node* newHeap = new Node[physicalSize];
            copy(heap, heap + virtualSize, newHeap);
            delete[] heap;
            heap = newHeap;
        }
    ...
    void print() {
        ...
        cout << "Элементы в порядке приоритета:" << endl;
        while (tempHeap.virtualSize > 0) {
            Node minNode = tempHeap.findMin();
            cout << "Приоритет: " << minNode.priority << ", Данные: " << minNode.data << endl;
            tempHeap.removeMin();
        }
    }
};
